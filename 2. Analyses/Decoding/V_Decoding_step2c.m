%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
% SQM_EEGdecoding
% Step 2c - Cross-condition decoding with V conditions (training with V4 vs NV, testing with V0 vs NV)
%==========================================================================
...
%% ========================================================================
% run the decoder 
% - NV vs V4 is now the training set  
for i = 1:numel(subjects)
    cd(main);cd(subjects(i).name);
    load(ls('*eeg_decoder.mat'));
    load(ls('*NV_V.mat'));
    % the W matrix needs to be loaded from decoder(3), in NV_V
    % the W is used to test the decoder in V0, V2
    list_cond       = [V0,  V2];
    name_cond       = {'V0','V2'};
    % update decoder options
    opt             = decoder(3).options;
    opt.st_dsig     = 0;
    opt.prec_W      = [];
    % keep giving NV as input to standardize the test set
    index           = ismember(sqmlabels,[NV V4]);
    Y{1}            = eegdata(:,:,index);
    X{1}            = sqmlabels(index);
    ...
    for k = 1:2  
        ...
        decoder(k).name     = ['NV-V4(train) vs NV-' name_cond{k} '(test)'];
        decoder(k).time     = time;
    ...
    % save
    fname           = [subjects(i).name '_LDA_NV_V4_train.mat'];
    save(fname,'decoder') 
...
load(ls('*NV_V.mat'))
NV_V4_train_LDA    = nan(numel(subjects),numel(decoder(3).time),...
                          numel(decoder(3).time),2);
for i = 1:numel(subjects)
cd(main);cd(subjects(i).name); 
    load(ls('*V4_train.mat'))
    for k = 1:2
        NV_V4_train_LDA(i,:,:,k) = decoder(k).results.tval;
    end
end
...
for k = 1:2
    subplot(2,2,k)
    imagesc(timevec,timevec,squeeze(mean(NV_V4_train_LDA(:,:,:,k))));
...
%% Cross-condition generalization - Results shown in Figure 2C (second plot)
nperms            = 10000;
...
for k = 1:2
    ...
    vline(0,'k');hline(0,'k');
    caxis([-6 6]);
    ShadePlotForEmpahsis([map1(1) map1(end)],[.5 .5 .5],.3);
    ShadePlotForEmpahsis([map2(1) map2(end)],[.5 .5 .5],.3)
    tl            = title(decoder(k).name); tl.FontWeight = 'normal';
    format_figure(nan,nan,'train time (ms)','test time (ms)');
end
